...
for typ in "${typs[@]}"
	do
	out='maps/worldmap_'$typ'_'$net'_'$param'_'$period'_year_'$model'.js'
	echo $out
	...
	plot: {
	cssClass: "mapLegend",
	title: "'$param2' Obs/'$model' - year averages - '$period'",
	mode: "horizontal",
	type: "circle",
	hideElemsOnClick: {
	opacity: 0,
	...
	for line in $list; do
		...
		#read stat file - MOD
		if [ -a './out/'$site'-'$param'_years_'$period'-'$model'.txt' ]; then
			l2=`cat './out/'$site'-'$param'_years_'$period'-'$model'.txt' | head -3 | tail -2`	
			read -a arr <<<$l2
			mod_trend=${arr[3]}
