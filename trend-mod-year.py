...
if len(datas)>0:
	...
	for y in np.linspace(years[1],years[-1],1+years[-1]-years[1]):
		...
		if not np.isnan(spring[i]) and not np.isnan(summer[i]) and not np.isnan(autumn[i]) and not np.isnan(winter[i]):
			...
			ydata=nanmean([spring[i],summer[i],autumn[i],winter[i]])
			ydatas.append(ydata)
	...
	for y in np.linspace(mod_years[1],mod_years[-1],1+mod_years[-1]-mod_years[1]):
		...
		if not np.isnan(mod_spring[i]) and not np.isnan(mod_summer[i]) and not np.isnan(mod_autumn[i]) and not np.isnan(mod_winter[i]):
			...
			mod_ydata=nanmean([mod_spring[i],mod_summer[i],mod_autumn[i],mod_winter[i]])
			mod_ydatas.append(mod_ydata)
	...
	if len(mod_sdatasok)>0:
		mod_l5=plt.plot_date(mod_yods,mod_ydatasok,color="#23d719", markersize=9, marker='x', markeredgewidth=1.5, alpha=0.8,zorder=8)
