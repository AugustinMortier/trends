...
#path, site, param, yrmin, yrmax, nmkmin = '/metno/aerocom/work/aerocom1/AEROCOM_OBSDATA/Export/Earlinet/data/', 'le', 'e532', 2000, 2014, 7
#path, site, param, yrmin, yrmax, nmkmin = '/metno/aerocom/work/aerocom1/AEROCOM_OBSDATA/Export/Earlinet/data/', 'lc', 'b355', 2002, 2012, 7
...
#print('ydatasok: ',np.shape(ydatasok))
#print('ydatas: ',np.shape(ydatas))

ydatasok=np.array(ydatasok)
ydatas=np.array(ydatas)

#print('ydatasok: ',ydatasok[:,1:20])
#print('ydatas: ',ydatas[:,1:20])
...
for y in years:
	#yearly average
	l2 = plt.plot(np.add(int(y),np.multiply(ydatas[i,:],(1/xmax))),z*1e-3,color="#2b2d39", linestyle='-', linewidth=1.5, alpha=0.35,zorder=2)

	#yearly ok average
	l3 = plt.plot(np.add(int(y),np.multiply(ydatasok[i,:],(1/xmax))),z*1e-3,color="#2b2d39", linestyle='-', linewidth=2.5, alpha=0.6,zorder=3)
	i = i + 1


#add title
ax.set_title(station,bbox=dict(facecolor='white', alpha=0.0, linewidth=0),weight='normal',color='.32')
...
# just to plot on the map: everything at 0
