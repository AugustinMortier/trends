...
if len(datas)>0:
    ...
    for season in seasons:
            ...
            if len(mod_sdatasok)>0:
                    mod_l5=plt.plot_date(mod_sodsok,mod_sdatasok,color="#fc9a00", markersize=9, marker='x', markeredgewidth=1.5, alpha=0.8,zorder=8)
                    mod_l6=plt.plot_date(mod_sodsok,mod_sdatasok,color="#fc9a00", linestyle='-', linewidth=2.5, markersize=0.0, markeredgewidth=0.0, alpha=0.6,zorder=7)
            ...
            if mod_p_stat=='yes':
                    if mod_pval<=1-sig:
                            ...
                            text(xmin+0.83*(xmax-xmin),ymin+0.75*(ymax-ymin),modtyp+': ' + mod_str_a + '%/yr',bbox=dict(facecolor='white', alpha=0.7, linewidth=0),color="#fc9a00",zorder=50)
                    else:
